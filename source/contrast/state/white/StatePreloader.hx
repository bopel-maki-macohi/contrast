...
class StatePreloader extends State
{
	...
	private function onPreloaderTick()
	{
		...
		if (done == totalTasks)
		{
			...
			final easterEggTime = Macro.getDefined('EASTER_EGG_AINSTANT') ? 1 : (10 + (Save.data.contrast / 4));
			trace('Easter Egg Wait Time: ${easterEggTime}s');
	...
	public static function moveToNextState()
	{
		...
		else if (STATE_NEXT == 'blue_start') FlxG.switchState(() -> new BlueStart());
